...
$(document).ready(function (){
    ...
    $(".signup").submit(function() {
        ...
        $.ajax({
            type: "POST", 
            url: "/gov-delivery/proxy_new.php",
            data: form.serialize(),
    ...
    $("a").click(function() {
        ...
        var interstitial = "<div class='screen' id='interScreen'><div class='modal'><p tabindex='0'>You’re leaving ConsumerFinance.gov to access a third-party website: <a href='"+url+"'>"+url+"</a>.</p><p><a id='interCancel' href='"+window.location+"'>Press ESC to cancel.</a></p><p class='fineprint'>We do not endorse or sponsor this website or its policies, products, or services. Our <a href='/privacy-policy/'>privacy policies</a> do not apply to this website.</p></div></div>"
        if(url.match("^(mailto:|\/|#)") || url.match("^tel:") || url.match("^javascript:") || url.match("^https?:\/\/[^\/]*\.gov") || url.match(host) || url.match('^\.')){
            return true;
        }
        else {
            $("body").append(interstitial);
            $("#interScreen p").get(0).focus();
            $("body").bind('keydown',escHandler);
            $("#interCancel").click(function(e) {
                e.preventDefault();
                window.clearTimeout(countdown);
                var target = $("#interScreen p:first a").attr("href");
                $("#interScreen").remove();
            ...
            countdown = window.setTimeout(function(link) { window.location = link; },7000,url);
            return false;
        }
    });
    $("a > img").each(function() {
